---
title: "Mesh Buffering Documentation"
description: "The HTTP buffering middleware in Mesh Proxy limits the size of requests that can be forwarded to Services. Read the technical documentation."
---
...
With Buffering, Mesh reads the entire request into memory (possibly buffering large requests into disk), and rejects requests that are over a specified size limit.
...
labels:
  - "Mesh.http.middlewares.limit.buffering.maxRequestBodyBytes=2000000"
```
...
```yaml tab="Docker & Swarm"
labels:
  - "Mesh.http.middlewares.limit.buffering.memRequestBodyBytes=2000000"
```
...
```yaml tab="Docker & Swarm"
labels:
  - "Mesh.http.middlewares.limit.buffering.maxResponseBodyBytes=2000000"
```
...
```yaml tab="Docker & Swarm"
labels:
  - "Mesh.http.middlewares.limit.buffering.memResponseBodyBytes=2000000"
```
...
??? example "Retries once in the case of a network error"
    ...
    ```yaml tab="Docker & Swarm"
    labels:
      - "Mesh.http.middlewares.limit.buffering.retryExpression=IsNetworkError() && Attempts() < 2"
    ```
