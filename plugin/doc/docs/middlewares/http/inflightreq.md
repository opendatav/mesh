---
title: "Mesh InFlightReq Documentation"
description: "Mesh Proxy's HTTP middleware lets you limit the number of simultaneous in-flight requests. Read the technical documentation."
---
...
The `ipStrategy` option defines three parameters that configures how Mesh determines the client IP: `depth`, `excludedIPs` and `ipv6Subnet`.
...
The `depth` option tells Mesh to use the `X-Forwarded-For` header and select the IP located at the `depth` position (starting from the right).
