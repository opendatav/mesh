---
title: "Mesh HTTP Middlewares IPWhiteList"
description: "Learn how to use IPWhiteList in HTTP middleware for limiting clients to specific IPs in Mesh Proxy. Read the technical documentation."
---
...
labels:
  - "Mesh.http.middlewares.test-ipwhitelist.ipwhitelist.sourcerange=127.0.0.1/32, 192.168.1.7"
```
...
The `ipStrategy` option defines two parameters that set how Mesh determines the client IP: `depth`, and `excludedIPs`.  
...
The `depth` option tells Mesh to use the `X-Forwarded-For` header and take the IP located at the `depth` position (starting from the right).
...
labels:
  - "Mesh.http.middlewares.test-ipwhitelist.ipwhitelist.sourcerange=127.0.0.1/32, 192.168.1.7"
  - "Mesh.http.middlewares.test-ipwhitelist.ipwhitelist.ipstrategy.depth=2"
```
...
`excludedIPs` configures Mesh to scan the `X-Forwarded-For` header and select the first IP not in the list.
...
labels:
    - "Mesh.http.middlewares.test-ipwhitelist.ipwhitelist.sourceRange=127.0.0.1/32, 192.168.1.0/24"
    - "Mesh.http.middlewares.test-ipwhitelist.ipwhitelist.ipstrategy.excludedips=127.0.0.1/32, 192.168.1.7"
```
...
labels:
  - "Mesh.http.middlewares.test-ipWhiteList.ipWhiteList.sourcecriterion.ipstrategy.ipv6Subnet=64"
```
