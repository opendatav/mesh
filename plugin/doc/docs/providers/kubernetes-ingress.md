...
The Mesh Kubernetes Ingress provider is a Kubernetes Ingress controller; that is to say,
...
When using a single instance of Mesh Proxy with Let's Encrypt, you should encounter no issues.
...
Early versions (v1.x) of Mesh used a [KV store](https://doc.Mesh.io/Mesh/v1.7/configuration/acme/#storage) to attempt to achieve this,
...
we recommend using [Mesh Enterprise](https://Mesh.io/mesh-enterprise/) which includes distributed Let's Encrypt as a supported feature.
...
```bash tab="CLI"
--providers.kubernetesingress.labelselector="app=Mesh"
```
...
Otherwise, Ingresses missing the annotation, having an empty value, or the value `Mesh` are processed.
...
```bash tab="CLI"
--providers.kubernetesingress.ingressclass=mesh-internal
```
...
Mesh will not discover cluster scope resources (`IngressClass` and `Nodes`).
...
For more information, please check out the `Mesh.ingress.kubernetes.io/service.nativelb` [service annotation documentation](../routing/providers/kubernetes-ingress.md#on-service).
...
To learn more about the various aspects of the Ingress specification that Mesh supports,
many examples of Ingresses definitions are located in the test [examples](https://github.com/Mesh/Mesh/tree/v3.4/pkg/provider/kubernetes/ingress/fixtures) of the Mesh repository.
