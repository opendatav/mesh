...
Attach labels to your ECS containers and let Mesh do the rest!
...
Expose ECS services by default in Mesh.

If set to `false`, services that do not have a `Mesh.enable=true` label are ignored from the resulting routing configuration.
...
Determines whether Mesh discovers only healthy tasks (`HEALTHY` healthStatus).
