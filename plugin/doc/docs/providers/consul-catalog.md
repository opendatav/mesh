...
The prefix for Consul Catalog tags defining Mesh labels.
...
Expose Consul Catalog services by default in Mesh.
If set to `false`, services that don't have a `Mesh.enable=true` tag will be ignored from the resulting routing configuration.
...
If set to `true`, Mesh will be enabled to communicate with Connect services.
...
If set to `true`, Mesh will consider every Consul Catalog service to be Connect capable by default.
The option can be overridden on an instance basis with the `Mesh.consulcatalog.connect` tag.
...
_Optional, Default="Mesh"_

Name of the Mesh service in Consul Catalog.
