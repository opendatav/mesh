---
title: "Mesh Logs Documentation"
description: "Logs are a key part of observability in Mesh Proxy. Read the technical documentation to learn their configurations, rotations, and time zones."
---

## Logs

Logs concern everything that happens to Mesh itself (startup, configuration, events, shutdown, and so on).
...
The section below describe how to configure Mesh logs using the static configuration.
...
The section below describes how to configure Mesh access logs using the static configuration.
...
| `accesslog.bufferingSize` | To write the logs in an asynchronous fashion, specify a  `bufferingSize` option.<br />This option represents the number of log lines Mesh will keep in memory before writing them to the selected output.<br />In some cases, this option can greatly help performances.| 0 | No      |
...
| `RouterName`  | The name of the Mesh  router.                                                                                                                                    |
| `ServiceName`    | The name of the Mesh backend.          |
| `ServiceURL`   | The URL of the Mesh backend.       |
| `ServiceAddr`    | The IP:port of the Mesh backend (extracted from `ServiceURL`). |
...
| `OriginStatus`          | The HTTP status code returned by the origin server. If the request was handled by this Mesh instance (e.g. with a redirect), then this value will be absent (0). |
...
| `RequestCount`          | The number of requests received since the Mesh instance started.    |
| `GzipRatio`             | The response body compression ratio achieved.   |
| `Overhead`              | The processing time overhead (in nanoseconds) caused by Mesh.    |
...
Mesh close and reopen its log files, assuming they're configured, on receipt of a USR1 signal.
...
Mesh will timestamp each log line in UTC time by default.

It is possible to configure the Mesh to timestamp in a specific timezone by ensuring the following configuration has been made in your environment:
