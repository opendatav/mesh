...
| `providers.nomad.constraints` | Defines an expression that Mesh matches against the container labels to determine whether to create any route for that container. See [here](#constraints) for more information.  |  ""   | No   |
| `providers.nomad.exposedByDefault` | Expose Nomad services by default in Mesh. If set to `false`, services that do not have a `Mesh.enable=true` tag will be ignored from the resulting routing configuration. See [here](../overview.md#restrict-the-scope-of-service-discovery) for additional information |  true    | No   |
...
| `providers.nomad.prefix` | Defines the prefix for Nomad service tags defining Mesh labels. | `Mesh`     | yes   |
| `providers.nomad.stale` | Instructs Mesh to use stale consistency for Nomad service API reads. See [here](#stale) for more information | false   | No   |
