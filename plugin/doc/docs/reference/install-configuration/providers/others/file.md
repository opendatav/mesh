...
| `providers.file.watch` | Set the `watch` option to `true` to allow Mesh to automatically watch for file changes. It works with both the `filename` and the `directory` options. | true | No |
