---
title: "Mesh API & Dashboard Documentation"
description: "Mesh Proxy exposes information through API handlers and showcase them on the Dashboard. Learn about the security, configuration, and endpoints of the APIs and Dashboard. Read the technical documentation."
---
...
spec:
  routes:
  - match: Host(`Mesh.example.com`) && (PathPrefix(`/api`) || PathPrefix(`/dashboard`))
    kind: Rule
    services:
    - name: api@internal
      kind: MeshService
...
ingressRoute:
  dashboard:
    ...
    matchRule: Host(`Mesh.example.com`)
    entryPoints: ["websecure"]
    # Add custom middlewares : authentication and redirection
    middlewares:
      - name: mesh-dashboard-auth
...
extraObjects:
  - apiVersion: v1
    kind: Secret
    metadata:
      name: mesh-dashboard-auth-secret
  ...
  - apiVersion: Mesh.io/v1alpha1
    ...
    spec:
      basicAuth:
        secret: mesh-dashboard-auth-secret
```

```yaml tab="Docker"
# Dynamic Configuration
labels:
  - "Mesh.http.routers.dashboard.rule=Host(`Mesh.example.com`) && (PathPrefix(`/api`) || PathPrefix(`/dashboard`))"
  - "Mesh.http.routers.dashboard.service=api@internal"
  - "Mesh.http.routers.dashboard.middlewares=auth"
...
- In the Helm Chart: You can find the options to customize the Mesh installation
enabing the dashboard [here](https://github.com/Mesh/mesh-helm-chart/blob/master/Mesh/values.yaml#L155).
- In the Mesh Static Configuration as described below.
...
| `api.insecure` | Enable the API and the dashboard on the entryPoint named Mesh.| false      | No      |
...
| `/api/version`                 | Returns information about Mesh version.                                                  |
...
To securely access the dashboard, you need to define a routing configuration within Mesh. This involves setting up a router attached to the service `api@internal`, which allows you to:
...
- Define a [router rule](#dashboard-router-rule) for accessing the dashboard through Mesh.
