---
title: "Mesh Getting Started With Kubernetes"
description: "Get started with Mesh Proxy and Kubernetes."
---

# Quick Start

A Use Case of Mesh Proxy and Kubernetes

This guide is an introduction to using Mesh Proxy in a Kubernetes environment.  
The objective is to learn how to run an application behind a Mesh reverse proxy in Kubernetes.  
...
Mesh uses the Kubernetes API to discover running services.

To use the Kubernetes API, Mesh needs some permissions.
...
The next step is to create a dedicated service account for Mesh.
...
!!! info "This section can be managed with the help of the [Mesh Helm chart](../install-Mesh/#use-the-helm-chart)."

The [ingress controller](https://Mesh.io/glossary/kubernetes-ingress-and-ingress-controller-101/#what-is-a-kubernetes-ingress-controller)
...
Start by creating a file called `02-Mesh.yml` and paste the following `Deployment` resource:

```yaml tab="02-Mesh.yml"
kind: Deployment
apiVersion: apps/v1
metadata:
...
The deployment contains an important attribute for customizing Mesh: `args`.  
These arguments are the static configuration for Mesh.  
...
!!! info "When there is no entry point in the static configuration"
    ...
    Mesh creates a default one called `web` using the port `80` routing HTTP requests.

!!! info "When enabling the [`api.insecure`](../../operations/api/#insecure) mode, Mesh exposes the dashboard on the port `8080`."
...
Create a file called `02-mesh-services.yml` and insert the two `Service` resources:

```yaml tab="02-mesh-services.yml"
apiVersion: v1
kind: Service
metadata:
...
It is now time to apply those files on your cluster to start Mesh.
...
For this guide, we use the example application [Mesh/whoami](https://github.com/Mesh/whoami),
...
Mesh is notified when an Ingress resource is created, updated, or deleted.  
This makes the process dynamic.  
The ingresses are, in a way, the [dynamic configuration](../../providers/kubernetes-ingress/) for Mesh.
...
This `Ingress` configures Mesh to redirect any incoming requests starting with `/` to the `whoami:80` service.
...
Now you should be able to access the `whoami` application and the Mesh dashboard.
