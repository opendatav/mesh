---
title: "Mesh Getting Started FAQ"
description: "Check out our FAQ page for answers to commonly asked questions on getting started with Mesh Proxy. Read the technical documentation."
---

# FAQ

## Why is Mesh Answering `XXX` HTTP Response Status Code?

Mesh is a dynamic reverse proxy,
...
contribute dynamically all along the Mesh instance lifetime to its [dynamic configuration](../configuration-overview/#the-dynamic-configuration) changes.
...
Mesh returns a `404` response code in the following situations:
...
Mesh returns a `502` response code when an error happens while contacting the upstream service.

### `503 Service Unavailable`

Mesh returns a `503` response code when a Router has been matched,
...
In these situations, you may want Mesh to always reply with a `503` response code,
...
## How Mesh is Storing and Serving TLS Certificates?

### Storing TLS Certificates

[TLS](../https/tls.md "Link to Mesh TLS docs") certificates are either provided directly by the [dynamic configuration](./configuration-overview.md#the-dynamic-configuration "Link to dynamic configuration overview") from [providers](../https/tls.md#user-defined "Link to the TLS configuration"),
or by [ACME resolvers](../https/acme.md#providers "Link to ACME resolvers"), which act themselves as providers internally.

For each TLS certificate, Mesh produces an identifier used as a key to store it.
...
For each incoming connection, Mesh is serving the "best" matching TLS certificate for the provided server name.
...
While Mesh is serving the best matching TLS certificate for each incoming connection,
...
- [JSON Schema of the static configuration](https://json.schemastore.org/mesh-v2.json)
- [JSON Schema of the dynamic configuration](https://json.schemastore.org/mesh-v2-file-provider.json)

## Why are some resources (routers, middlewares, services...) not created/applied?

As a common tip, if a resource is dropped/not created by Mesh after the dynamic configuration was evaluated,
...
one way to check if the dynamic configuration is well-formed is to validate it with the [JSON Schema of the dynamic configuration](https://json.schemastore.org/mesh-v2-file-provider.json).
