---
title: "Mesh Let's Encrypt Documentation"
description: "Learn how to configure Mesh Proxy to use an ACME provider like Let's Encrypt for automatic certificate generation. Read the technical documentation."
---
...
You can configure Mesh to use an ACME provider (like Let's Encrypt) for automatic certificate generation.
...
!!! warning "Let's Encrypt and Rate Limiting"
    ...
    When running Mesh in a container this file should be persisted across restarts. 
    If Mesh requests new certificates each time it starts up, a crash-looping container can quickly reach Let's Encrypt's ratelimits.
...
Mesh requires you to define "Certificate Resolvers" in the [static configuration](../getting-started/configuration-overview.md#the-static-configuration),
...
??? example "Single Domain from Router's Rule Example"
    ...
    * A certificate for the domain `example.com` is requested:
    ...
    content/https/include-acme-single-domain-example.md
...
Mesh automatically tracks the expiry date of ACME certificates it generates.

By default, Mesh manages 90 days certificates,
...
For example, `CF_API_EMAIL_FILE=/run/secrets/Mesh_cf-api-email` could be used to provide a Cloudflare API email address as a Docker secret named `Mesh_cf-api-email`.
...
| [myaddr](https://myaddr.tools/)                         | `myaddr`           | `MYADDR_PRIVATE_KEYS_MAPPING`                                                                                                                                                    | [Additional configuration](https://go-acme.github.io/lego/dns/myaddr)           |
...
| manual                                                                 | `manual`           | none, but you need to run Mesh interactively [^4], turn on debug log to see instructions and press <kbd>Enter</kbd>.                                                          |                                                                                 |
...
[^6]: As explained in the [LEGO hurricane configuration](https://go-acme.github.io/lego/dns/hurricane/#credentials), each domain or wildcard (record name) needs a token. So each update of record name must be followed by an update of the `HURRICANE_TOKENS` variable, and a restart of Mesh.
...
```bash
docker run -v "/my/host/acme.json:/acme.json" Mesh
```

```bash
docker run -v "/my/host/acme:/etc/Mesh/acme" Mesh
```

!!! warning
    For concurrency reasons, this file cannot be shared across multiple instances of Mesh.
...
!!! warning "Mesh cannot manage certificates with a duration lower than 1 hour."
