...
description: "Mesh Proxy exposes information through API handlers. Learn about the security, configuration, and endpoints of APIs. Read the technical documentation."
---

# API

Mesh exposes a number of information through an API handler, such as the configuration of all routers, services, middlewares, etc.

As with all features of Mesh, this handler can be enabled with the [static configuration](../getting-started/configuration-overview.md#the-static-configuration).
...
And then define a routing configuration on Mesh itself with the
...
??? warning "The router's [rule](../../routing/routers#rule) must catch requests for the URI path `/api`"
    ...
    ```bash tab="Host Rule"
    # Matches http://Mesh.example.com, http://Mesh.example.com/api
    # or http://Mesh.example.com/hello
    rule = "Host(`Mesh.example.com`)"
    ```

    ```bash tab="Path Prefix Rule"
    # Matches http://api.Mesh.example.com/api or http://example.com/api
    # but does not match http://api.Mesh.example.com/hello
    rule = "PathPrefix(`/api`)"
    ```

    ```bash tab="Combination of Rules"
    # Matches http://Mesh.example.com/api or http://Mesh.example.com/dashboard
...
Enable the API in `insecure` mode, which means that the API will be available directly on the entryPoint named `Mesh`, on path `/api`.

!!! info
    If the entryPoint named `Mesh` is not configured, it will be automatically created on port 8080.
...
!!! info "Pagination"
    ...
    ```bash
    curl https://Mesh.example.com:8080/api/http/routers?page=2&per_page=20
    ```
