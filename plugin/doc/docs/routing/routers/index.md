---
title: "Mesh Routers Documentation"
...
!!! info "Expressing Complex Rules Using Operators and Parenthesis"
    ...
    - host is `example.org` AND path is NOT `/Mesh`

    ```yaml
    Host(`example.com`) || (Host(`example.org`) && !Path(`/Mesh`))
    ```
...
??? warning "Maximum Value"
    ...
    Mesh reserves a range of priorities for its internal routers,
...
??? example "Set priorities -- using the [File Provider](../../providers/file.md)"
    ...
    In this configuration, the priority is configured to allow `Router-2` to handle requests with the `foobar.Mesh.com` host.

### RuleSyntax

_Optional, Default=""_

In Mesh v3 a new rule syntax has been introduced ([migration guide](../../migration/v2-to-v3.md#router-rule-matchers)).
...
??? example "Set rule syntax -- using the [File Provider](../../providers/file.md)"
    ...
    ```yaml tab="Kubernetes Mesh.io/v1alpha1"
...
When a TLS section is specified, it instructs Mesh that the current router is dedicated to HTTPS requests only
...
!!! info "Domain Fronting"
    ...
    if the TLS options associated with the Host Header and the SNI are different then Mesh will respond with a status code `421`.
...
If `certResolver` is defined, Mesh will try to generate certificates based on routers `Host` & `HostSNI` rules.
...
??? info "How to handle Server First protocols?"
    ...
    To correctly handle a request, Mesh needs to wait for the first few bytes to arrive before it can decide what to do with it.
    ...
    The only way that Mesh can deal with such a case, is to make sure that on the concerned entry point,
