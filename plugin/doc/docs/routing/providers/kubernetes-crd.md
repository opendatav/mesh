---
title: "Routing Configuration for Mesh CRD"
description: "Understand the routing configuration for the Kubernetes IngressRoute & Mesh CRD. Read the technical documentation."
---
...
* You can find an exhaustive list, generated from Mesh's source code, of the custom resources and their attributes in [the reference page](../../reference/dynamic-configuration/kubernetes-crd.md).
* Validate that [the prerequisites](../../providers/kubernetes-crd.md#requirements) are fulfilled before using the Mesh custom resources.
* Mesh CRDs are building blocks that you can assemble according to your needs.
...
| [MeshService](#kind-Meshservice)           | Abstraction for HTTP loadbalancing/mirroring                       | [HTTP service](../services/index.md#configuring-http-services) |
...
| [ServersTransport](#kind-serverstransport)       | Allows to configure the transport between Mesh and the backends | [ServersTransport](../../services/#serverstransport_1)         |
| [ServersTransportTCP](#kind-serverstransporttcp) | Allows to configure the transport between Mesh and the backends | [TCP ServersTransport](../../services/#serverstransport_3)     |

### Kind: `IngressRoute`

`IngressRoute` is the CRD implementation of a [Mesh HTTP router](../routers/index.md#configuring-http-routers).
...
| [12] | `routes[n].services`           | List of any combination of [MeshService](#kind-Meshservice) and reference to a [Kubernetes service](https://kubernetes.io/docs/concepts/services-networking/service/) (See below for `ExternalName Service` setup)                                                                     |
...
!!! important "Configuring Backend Protocol"
    ...
    There are 3 ways to configure the backend protocol for communication between Mesh and your pods:
	
    - Setting the scheme explicitly (http/https/h2c)
    - Configuring the name of the kubernetes service port to start with https (https)
    - Setting the kubernetes service port to use port 443 (https)

    If you do not configure the above, Mesh will assume an http connection.
    

!!! important "Using Kubernetes ExternalName Service"

    Mesh backends creation needs a port to be set, however Kubernetes [ExternalName Service](https://kubernetes.io/docs/concepts/services-networking/service/#externalname) could be defined without any port.
...
`Middleware` is the CRD implementation of a [Mesh middleware](../../middlewares/http/overview.md).
...
`MeshService` is the CRD implementation of a ["Mesh Service"](../services/index.md).

Register the `MeshService` [kind](../../reference/dynamic-configuration/kubernetes-crd.md#definitions) in the Kubernetes cluster before creating `MeshService` objects,
referencing services in the [`IngressRoute`](#kind-ingressroute) objects, or recursively in others `MeshService` objects.

!!! info "Disambiguate Mesh and Kubernetes Services"
    ...
    * `MeshService`: to reference another [Mesh Service](../services/index.md)

`MeshService` object allows to use any (valid) combinations of:
...
??? "Declaring and Using Mirroring"
    ...
    ```yaml tab="Mirroring Mesh Service"
...
!!! important "References and namespaces"
    ...
    Additionally, when the definition of the `MeshService` is from another provider,
    the cross-provider syntax (`service@provider`) should be used to refer to the `MeshService`, just as in the middleware case.
...
`IngressRouteTCP` is the CRD implementation of a [Mesh TCP router](../routers/index.md#configuring-tcp-routers).
...
`MiddlewareTCP` is the CRD implementation of a [Mesh TCP middleware](../../middlewares/tcp/overview.md).
...
`IngressRouteUDP` is the CRD implementation of a [Mesh UDP router](../routers/index.md#configuring-udp-routers).
...
`TLSOption` is the CRD implementation of a [Mesh "TLS Option"](../../https/tls.md#tls-options).
...
| [9]  | `sniStrict`                 | if `true`, Mesh won't allow connections from clients connections that do not specify a server_name extension.                                                                                                           |
...
`TLSStore` is the CRD implementation of a [Mesh "TLS Store"](../../https/tls.md#certificates-stores).

Register the `TLSStore` kind in the Kubernetes cluster before creating `TLSStore` objects.

!!! important "Default TLS Store"

    Mesh currently only uses the [TLS Store named "default"](../../https/tls.md#certificates-stores).
    This _default_ `TLSStore` should be in a namespace discoverable by Mesh. Since it is used by default on [`IngressRoute`](#kind-ingressroute) and [`IngressRouteTCP`](#kind-ingressroutetcp) objects, there never is a need to actually reference it.
