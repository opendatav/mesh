...
description: "For routing and load balancing in Mesh Proxy, EntryPoints define which port will receive packets and whether in UDP or TCP. Read the technical documentation."
---
...
EntryPoints are the network entry points into Mesh.
...
`allowACMEByPass` determines whether a user defined router can handle ACME TLS or HTTP challenges instead of the Mesh dedicated one.
...
??? example "Listen on the same port"
    ...
    Now it is possible to run multiple Mesh processes with the same EntryPoint configuration.
...
??? info "HTTP/3 uses UDP+TLS"
    ...
    As HTTP/3 actually uses UDP, when Mesh is configured with a TCP entryPoint on port N with HTTP/3 enabled,
...
It can be used to override the authority in the `alt-svc` header, for example if the public facing port is different from where Mesh is listening.
...
You can configure Mesh to trust the forwarded headers information (`X-Forwarded-*`).
...
??? info "`forwardedHeaders.connection`"
    ...
    As per RFC7230, Mesh respects the Connection options from the client request.
...
`respondingTimeouts` are timeouts for incoming requests to the Mesh instance.
...
Controls the behavior of Mesh during the shutdown phase.
...
??? info "`lifeCycle.graceTimeOut`"
    ...
    Duration to give active requests a chance to finish before Mesh stops.
...
The maximum number of requests Mesh can handle before sending a `Connection: Close` header to the client (for HTTP2, Mesh sends a GOAWAY). Zero means no limit.
...
The maximum duration Mesh can handle requests before sending a `Connection: Close` header to the client (for HTTP2, Mesh sends a GOAWAY). Zero means no limit.
...
Mesh supports [PROXY protocol](https://www.haproxy.org/download/2.0/doc/proxy-protocol.txt) version 1 and 2.
...
??? info "`proxyProtocol.insecure`"
    ...
    In a test environments, you can configure Mesh to trust every incoming connection.
...
!!! warning "Queuing Mesh behind Another Load Balancer"

    When queuing Mesh behind another load-balancer, make sure to configure PROXY protocol on both sides.
...
Mesh supports [systemd socket activation](https://www.freedesktop.org/software/systemd/man/latest/systemd-socket-activate.html).
...
```bash
systemd-socket-activate -l 80 -l 443 --fdname web:websecure  ./Mesh --entrypoints.web --entrypoints.websecure
```
